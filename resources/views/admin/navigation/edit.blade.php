...
@section('title', 'Editar Enlace de Menú')

@section('breadcrumbs')
    <span class="text-gray-400">Panel</span> / <a href="{{ route('navigation.index') }}" class="text-gray-400 hover:text-[var(--primary)]">Navegación</a> / <span class="text-[var(--primary)]">Editar</span>
@endsection
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-extrabold text-[var(--primary-dark)] font-montserrat">
            Editar Enlace de Navegación
        </h1>
        <p class="text-xs text-gray-500 font-semibold mt-1">
            Modifica la etiqueta o la URL de destino de este enlace.
        </p>
    </div>

    <div class="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
        <form action="{{ route('navigation.update', $navigation->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <x-select label="Ubicación del Enlace" name="location" required="true" help="Dónde se renderizará el enlace.">
                    <option value="header" {{ $navigation->location === 'header' ? 'selected' : '' }}>Solo Encabezado (Header)</option>
                    <option value="footer" {{ $navigation->location === 'footer' ? 'selected' : '' }}>Solo Pie de Página (Footer)</option>
                    <option value="both" {{ $navigation->location === 'both' ? 'selected' : '' }}>Ambas Ubicaciones</option>
                </x-select>

                <!-- Target window -->
                <x-select label="Destino de Apertura" name="target" required="true" help="Cómo abrirá el navegador el enlace.">
                    <option value="_self" {{ $navigation->target === '_self' ? 'selected' : '' }}>Misma Ventana / Pestaña</option>
                    <option value="_blank" {{ $navigation->target === '_blank' ? 'selected' : '' }}>Nueva Pestaña / Ventana</option>
                </x-select>
            </div>
